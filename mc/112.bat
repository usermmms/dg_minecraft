...
if "%getcrtsha256%" == "" ( title error code : dg6 & echo error code : dg6 & echo 错误！hash不能为空！ & echo 可能原因： & echo 网络不稳定或者维护中... & echo 按任意键尝试重新下载 或联系管理员 & pause & goto dgcrtall)
...
) else ( echo ?证书包不完整或已被更改!!!  尝试重新下载或联系管理员 & echo 按任意键尝试重新下载 & pause & goto dgcrtall
)
...
echo 输入 3 略过安装额外工具包 仅建议网络非常差时选择
set /p toolsin=
if "%tools%"=="1" (
cls
echo 您选择安装完整工具包！ 
...
if "%reshash%" == "%getreshash%" ( echo 校验成功! & powershell -command "Expand-Archive -Force '%~dp0./temp/resourcepacks.zip' '%~dp0./.minecraft/resourcepacks/'" & goto maininstall 
 ) else ( 
...
if "%shashahash%" == "%getshashahash%" ( echo 校验成功. & powershell -command "Expand-Archive -Force '%~dp0./temp/shaderpacks.zip' '%~dp0./.minecraft/shaderpacks/'" & goto miniinstall
 ) else ( 
...
if "%gamehash%" == "%getgamehash%" ( echo 校验成功！ & powershell -command "Expand-Archive -Force '%~dp0./temp/game.zip' '%~dp0./'" & goto dgmods
 ) else ( 
...
for /F %%i in ('powershell -command ./.minecraft/apps/tools/cat.exe ./temp/gamesha256.txt') do @set getgamehash=%%i
if "%getgamehash%" == "" ( title error code : dg11 & echo error code : dg11 & echo 下载游戏本体失败！ & echo 按任意键尝试再次下载  如遇再次失败 请联系管理员 & pause & goto miniinstall )
if "%gamehash%" == "%getgamehash%" ( echo 校验成功！ & powershell -command "Expand-Archive -Force '%~dp0./temp/game.zip' '%~dp0./'" & goto dgmods
 ) else ( 
title error:dg11 & title error:dg11 & echo 下载游戏本体时多次被更改或不完整！ & echo 按任意键尝试再次下载 如遇再次下载失败 请联系管理员  & pause & goto miniinstall )
...
if "%modshash%" == "%getmodshash%" ( echo 校验成功！ & powershell -command "Expand-Archive -Force '%~dp0./temp/mods.zip' '%~dp0./.minecraft/mods/'" & goto dgupdate1
 ) else ( 
...
if "%update1hash%" == "%getupdate1hash%" ( echo 校验成功！ & powershell -command "Expand-Archive -Force '%~dp0./temp/update1.zip' '%~dp0./'" goto inend
 ) else ( 
...
for /F %%i in ('powershell -command ./.minecraft/apps/tools/cat.exe ./temp/update1sha256.txt') do @set getupdate1hash=%%i
if "%getupdate1hash%" == "" ( title error code : dg13 & echo error code : dg13 & echo 下载更新内容失败！ & echo 按任意键尝试再次下载  如遇再次失败 请联系管理员 & pause & goto dgupdate1 )
if "%update1hash%" == "%getupdate1hash%" ( echo 校验成功！ & powershell -command "Expand-Archive -Force '%~dp0./temp/update1.zip' '%~dp0./'" goto inend
 ) else ( 
title error:dg13 & title error:dg13 & echo 下载更新内容多次被更改或不完整！ & echo 按任意键尝试再次下载 如遇再次下载失败 请联系管理员  & pause & goto dgupdate13 )
